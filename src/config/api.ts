import { IBackendRes, ICompany, IAccount, IUser, IModelPaginate, IGetAccount, IJob, IResume, IPermission, IRole, ISkill, ISubscribers, IComment, IResCommentDTO } from '@/types/backend';
...
export const callFetchJobAD = (query: string) => {
    return axios.get<IBackendRes<IModelPaginate<IJob>>>(`/api/v1/adjobs?${query}`);
}
...
Module Comment
 */
export const callCreateComment = (jobId: string, content: string) => {
    return axios.post<IBackendRes<IComment>>(`/api/v1/comments/${jobId}`, content, {
        headers: {
...
export const callUpdateComment = (id: string, content: string) => {
    return axios.put<IBackendRes<IComment>>(`/api/v1/comments/${id}`, content, {
        headers: {
            'Content-Type': 'text/plain'
        }
    });
...
export const callDeleteComment = (id: string) => {
    return axios.delete<IBackendRes<void>>(`/api/v1/comments/${id}`);
}

export const callFetchCommentsByJobId = (jobId: string) => {
    return axios.get<IBackendRes<IResCommentDTO[]>>(`/api/v1/comments/${jobId}`);
}
